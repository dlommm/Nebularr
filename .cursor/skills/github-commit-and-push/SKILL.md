...
description: >-
  ...
  Docker Hub with the latest tag plus the current semver as a multi-platform
  build (linux/amd64 and linux/arm64 by default; see scripts/docker-release-build.sh),
  unless the user opts out or Docker is unavailable. After that, by default creates a Git tag
...
7. **Docker Hub — required for “push to GitHub” (Nebularr app image):** immediately after step **5** succeeds, run the **Docker Hub publish** section below. **Do not skip** this step for a normal “push to GitHub” / “push to origin” / “sync the repo” request. Use **`./scripts/docker-release-build.sh --push`** so **amd64 and arm64** are published (see that script’s `PLATFORMS`); do not substitute a single-arch `docker build` for Hub.
   ...
   - If **build or push fails** (e.g. not logged in to Docker Hub, network error, multi-platform buildx/binfmt), **report the error**, suggest `docker login`, builder/QEMU steps from the **Docker Hub publish** section, and retry; **do not** claim the image is published.
...
2. From the **repository root** (where the root `Dockerfile` lives), **build and push** a **multi-platform** image so `docker pull` works on both **x86_64/amd64** and **arm64** (default in the release script: `linux/amd64` and `linux/arm64`). The registry stores a **manifest list**; tags remain **`latest`** and **`$APP_VERSION`**.
3. **Do not** publish a **single-arch-only** image to Hub for Nebularr unless the user explicitly asked (e.g. emergency one-arch hotfix).

**Required (Nebularr):** use **`./scripts/docker-release-build.sh --push`**. The script:
- Pushes **amd64 + arm64** by default (override with env **`PLATFORMS`** if needed; see script header).
- Omits SLSA provenance / full SBOM by default to **reduce Docker Hub Scout** noise.
- **Before pushing:** run `cd frontend && npm run build` if the WebUI changed so `src/arrsync/web/dist` matches the commit.

```bash
cd /path/to/Nebularr
./scripts/docker-release-build.sh --push
# Optional: IMAGE=youruser/nebularr ./scripts/docker-release-build.sh --push
# Optional: PLATFORMS=linux/amd64,linux/arm64 ./scripts/docker-release-build.sh --push
```

**If multi-platform build fails (agent troubleshooting):**
- `docker buildx ls` — use a **docker-container**-style builder if the default cannot emit a manifest list; e.g. `docker buildx create --name nebularr-multi --use --bootstrap`.
- On **Linux** without QEMU/binfmt: `docker run --privileged tonistiigi/binfmt --install all` (or distro packages) so **non-native** platforms in the list can build.
- If the user must ship **one** arch temporarily, set **`PLATFORMS=linux/amd64`** (or `linux/arm64` only) and **state** that in the reply. Default for normal releases remains **both**.

**`--load` (local smoke test):** `./scripts/docker-release-build.sh --load` is **single-platform** (native); it does not replace a Hub push.

**Alternative (manual `docker buildx build`)** only if the script is unavailable: include **`--platform linux/amd64,linux/arm64`** with **`--push`**, same `APP_VERSION` / `GIT_SHA` / no-attestation flags as the script. Use `read_text()` (not `read_bytes()`) for `tomllib` on Python 3.14+.
...
- **pgAdmin** or other **secondary** images: only if the user asked; default is the **Nebularr** app image only.

**Output to the user after Docker push**

- Image name(s), tags (**`latest`** and **`x.y.z`**), **`APP_VERSION` / `GIT_SHA`**, and **architectures** (e.g. **linux/amd64, linux/arm64**).
...
- **Docker Hub:** by default, confirm **image**, **`latest`** and **semver** tags, **platforms** (amd64 + arm64), and build **metadata** (version + git sha), or explain **skip** / **failure** and next steps.
...
- **Docker Hub** — **not optional** for a normal “push to GitHub” on Nebularr: the agent must run **Docker Hub publish** after a successful `git push` (see **step 7**), unless a skip reason applies. Use **`./scripts/docker-release-build.sh --push`** so **`linux/amd64`** and **`linux/arm64`** are published (manifest list); that keeps `latest` aligned with the branch and avoids single-arch pull failures on the other architecture.
