...
- **hub.docker.com** (Docker Scout UI) can show **more** total CVEs than a minimal runtime (e.g. **layer breakdown**, **base image**, **and** attestation / SBOM metadata on tagged images). **Pushes** from this repo should use **`./scripts/docker-release-build.sh --push`**, which publishes **linux/amd64** and **linux/arm64** by default and omits SLSA provenance / full SBOM by default; that **reduces** spurious package rows and matches how operators pull on both architectures. Trade-off: less supply-chain attestation on the image.
