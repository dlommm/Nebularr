...
| Document | What it covers (detailed) |
|----------|---------------------------|
| [**DB_BOOTSTRAP.md**](https://github.com/OWNER/REPO/blob/main/docs/DB_BOOTSTRAP.md) | **First boot:** sequence from Postgres container start through `docker/postgres/init` role creation, **Alembic upgrade** on app startup, and which schemas/tables appear. Includes **verification SQL** (`\du`, `\dn`, sample counts). The minimal reference for “what happens the first time Postgres comes up with Nebularr.” |
