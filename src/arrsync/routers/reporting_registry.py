...
Every dashboard panel whose body is a single SQL query lives here as a
``PanelSpec``: the dashboard GET handlers compose their row/scalar panels from
this registry, and the CSV export executes *exactly one* panel's SQL through it
...
* The six near-identical distribution panels (quality / codec / language / size
  band, over the episode or movie file views) are generated by
  :func:`distribution_by_view` rather than copy-pasted.
"""
...
def build_scalar(session: Any, spec: PanelSpec, params: Params) -> Any:
    sql, binds = spec.build(params)
    return session.execute(text(sql), binds).scalar_one()
...
# the registry. (queue_status_breakdown/sync_state_snapshot read app.webhook_queue
# and app.sync_state, which are not instance-scoped, hence no filter.)
